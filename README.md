...
Payments / Finance v1 adds settlement visibility at `/app/finance` and `/admin/finance`: captured, pending, failed, delivered, and refund-review payment posture is computed from existing order, job, payment, payout, and support escalation records. Finance v1.1 adds persistent finance review records so business operators can assign owners, move reviews through `OPEN`, `IN_REVIEW`, `WAITING_SUPPORT`, `RESOLVED`, or `CANCELLED`, and document manual closeout. No automated refunds, payout automation, destructive payment mutation, or payment-provider changes are included.
